// Ejercicio 1: Crea un parseador que reciba un string en formato CSV y devuelva una colección de objetos.
...
const fromCSVClasic = (csv, nAttrs) => {
  ...
  if (
    Number.isNaN(nAttrs) ||
    !Number.isInteger(nAttrs) ||
    nAttrs < 0 ||
    nAttrs > headings.length
  ) {
    nAttrs = headings.length;
  }

  for (let i = 1; i < allTextLines.length; i++) {
    const values = allTextLines[i].split(",");
    const student = {};
...
const fromCSVFunctional = (csv, nAttrs) => {
  const [keys, ...rows] =
    csv?.split("\n").map((row) => row.split(",", nAttrs)) ?? [];
  return rows?.map((row) =>
    Object.fromEntries(row.map((value, i) => [keys[i], value]))
  );
};

const result1 = fromCSVClasic(data, 9);
const result2 = fromCSVFunctional(data, 8);
//console.log(result1);
//console.log(result2);

// Ejercicio 2: Implementar una funcion replaceAt que tome como primer argumento un array, como segundo argumento un índice
...
const replaceAt = (arr, index, newValue) => {
  let arrResult = [...arr];
  arrResult[index] = newValue;
  return arrResult;
};
...
// Ejercicio 3: Crea una función que reciba una colección de objetos y un año.
// Dicha función deberá de mostrar los nombres de las tres personas con el ranking más alto del año.
...
// Ejercicio 4: Crear funcion para normalizar una colección de objetos a un objeto,
// de tal manera que devuelva un objeto que tenga como claves las ids de los objetos y como valores los objetos en sí pero sin la id.
    ...
    row.id,
      {
        ...
        capital: row.capital,
        national_sport: row.national_sport,
        flag: row.flag,
      };
  });
...
const result33 = normalize(collection);
